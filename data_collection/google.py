...
def google_search(query, num_results=2):
    ...
    """
    try:
        # Check the correct parameter name in the library documentation
        search_results = list(search(query, num=num_results))  # Changed from num_results to num
...
if __name__ == "__main__":
    ...
    for i, result in enumerate(results):
        print(f"\n--- Result {i+1} ---")
        # Check if result is a dictionary before trying to access keys
        if isinstance(result, dict):
            print(f"URL: {result.get('url', 'No URL')}")
            content_preview = result.get('content', 'No content')[:200] if result.get('content') else 'No content'
